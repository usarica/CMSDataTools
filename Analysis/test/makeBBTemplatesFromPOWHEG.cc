...
// Process handle
typedef BBProcessHandler ProcessHandleType;
const ProcessHandleType& theProcess = TemplateHelpers::OffshellBBProcessHandle;

// Process-specific functions
void makeBBTemplatesFromPOWHEG_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeBBTemplatesFromPOWHEG_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeBBTemplatesFromPOWHEG_checkstage(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const unsigned int istage, const TString fixedDate="");
...
CheckStageFcn checkstagefcn = &makeBBTemplatesFromPOWHEG_checkstage;
#endif
...
void makeBBTemplatesFromPOWHEG_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  strSampleIdentifiers.push_back("bb_Sig_POWHEG");
...
void makeBBTemplatesFromPOWHEG_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  if (gSystem->AccessPathName(cinput)) makeBBTemplatesFromPOWHEG_one(channel, category, hypo, syst, fixedDate);
...
void makeBBTemplatesFromPOWHEG_checkstage(
  ...
  for (unsigned int t=0; t<ntpls; t++){
    ...
    TFile*& finput = finputList.at(ProcessHandleType::castHypothesisTypeToInt(treetype)>=ProcessHandleType::castHypothesisTypeToInt(ProcessHandleType::nBBSMTypes));
    finput->cd();
