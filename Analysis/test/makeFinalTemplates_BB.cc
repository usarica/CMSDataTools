#ifndef MAKEFINALTEMPLATES_BB_H
#define MAKEFINALTEMPLATES_BB_H
...
void makeFinalTemplates_BB(const Channel channel, const ACHypothesis hypo, const SystematicVariationTypes syst, CategorizationHelpers::MassRegion massregion, const unsigned int istage=1, const TString fixedDate=""){
  const ProcessHandler::ProcessType proctype=ProcessHandler::kBB;
  if (channel==NChannels) return;
...
template<> void getTemplatesPerCategory<2>(
  ...
  for (unsigned int t=0; t<ntpls; t++){
    ...
    TString templatename = thePerProcessHandle->getTemplateName(tpltype);
    TString templatetitle = thePerProcessHandle->getProcessLabel(tpltype, hypo);
    treeList.emplace_back(templatename+"_Smooth", "", tree, KDvars.find(KDset.at(0))->second, KDvars.find(KDset.at(1))->second, weight, isCategory);
  }
...
template<> void getTemplatesPerCategory<3>(
  ...
  for (unsigned int t=0; t<ntpls; t++){
    ...
    TString templatename = thePerProcessHandle->getTemplateName(tpltype);
    TString templatetitle = thePerProcessHandle->getProcessLabel(tpltype, hypo);
    treeList.emplace_back(templatename+"_Smooth", "", tree, KDvars.find(KDset.at(0))->second, KDvars.find(KDset.at(1))->second, KDvars.find(KDset.at(2))->second, weight, isCategory);
  }
  vector<TH_t*> hSmoothList = getSimultaneousSmoothHistograms(
