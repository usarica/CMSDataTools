  ...
  unordered_map<Category, vector<ExtendedHistogram_1D>, std::hash<int>> hMass_FromNominalInclusive; // [tree]
  {
    ...
    for (unsigned int it=0; it<treeList.size(); it++){
      ...
      for (int ev=0; ev<tree->GetEntries(); ev++){
        ...
        float constexpr inclusivenorm=1; // If normalization comes from something other than POWHEG, change this line to non-constexpr and the two commented lines below should be uncommmented.
        float one=1;
      ...
      if (massregion==kOnshell){
        ...
        for (Category& cat:catList){
          getSmoothHistogram(hMass_FromNominalInclusive[cat].at(it).getHistogram(), binning_hmass_list[cat], (cat==JJVBFTagged || cat==HadVHTagged ? 2 : 4));
