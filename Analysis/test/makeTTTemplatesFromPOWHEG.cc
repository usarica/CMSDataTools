...
// Process handle
typedef TTProcessHandler ProcessHandleType;
const ProcessHandleType& theProcess = TemplateHelpers::OffshellTTProcessHandle;

// Process-specific functions
void makeTTTemplatesFromPOWHEG_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeTTTemplatesFromPOWHEG_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeTTTemplatesFromPOWHEG_checkstage(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const unsigned int istage, const TString fixedDate="");
...
CheckStageFcn checkstagefcn = &makeTTTemplatesFromPOWHEG_checkstage;
#endif
...
void makeTTTemplatesFromPOWHEG_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  strSampleIdentifiers.push_back("tt_Sig_POWHEG");
...
void makeTTTemplatesFromPOWHEG_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  if (gSystem->AccessPathName(cinput)) makeTTTemplatesFromPOWHEG_one(channel, category, hypo, syst, fixedDate);
...
void makeTTTemplatesFromPOWHEG_checkstage(
  ...
  for (unsigned int t=0; t<ntpls; t++){
    ...
    TFile*& finput = finputList.at(ProcessHandleType::castHypothesisTypeToInt(treetype)>=ProcessHandleType::castHypothesisTypeToInt(ProcessHandleType::nTTSMTypes));
    finput->cd();
