...
const ProcessHandleType& theProcess = TemplateHelpers::OffshellZHProcessHandle;

// Process-specific functions
void makeZHTemplatesFromJHUGen_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeZHTemplatesFromJHUGen_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate="");
void makeZHTemplatesFromJHUGen_checkstage(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const unsigned int istage, const TString fixedDate="");
...
CheckStageFcn checkstagefcn = &makeZHTemplatesFromJHUGen_checkstage;
#endif
...
void plotProcessCheckStage_SystPairs(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const unsigned int istage, const TString fixedDate="", ProcessHandler::ProcessType proctype=theProcess.getProcessType(), const TString strGenerator="JHUGen");
...
void makeZHTemplatesFromJHUGen_one(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  TString OUTPUT_NAME = Form(
    "HtoZZ%s_%s_%s_FinalTemplates_%s_%s_JHUGen",
  ...
  strSampleIdentifiers.push_back("ZH_Sig_JHUGen");
  ...
  SystematicsClass* systhandle = constructSystematic(category, channel, theProcess.getProcessType(), syst, theSampleSet->getCJLSTTreeList(), extraEvaluators, "JHUGen");
  ...
  for (unsigned int t=0; t<tplset.size(); t++){
    ...
    // Build the MELA reweightings separately for each JHUGen 2f2f' sample set since NormComponent combines xsec
    ...
    for (auto& tree:theSampleSet->getCJLSTTreeList()) melarewgtBuilder->setupWeightVariables(tree, 0.9999, 0);
...
void makeZHTemplatesFromJHUGen_two(const Channel channel, const Category category, const ACHypothesis hypo, const SystematicVariationTypes syst, const TString fixedDate){
  ...
  if (gSystem->AccessPathName(cinput)) makeZHTemplatesFromJHUGen_one(channel, category, hypo, syst, fixedDate);
...
void makeZHTemplatesFromJHUGen_checkstage(
  ...
  for (unsigned int f=0; f<2; f++){
    ...
    TString INPUT_NAME = Form(
      "HtoZZ%s_%s_%s_FinalTemplates_%s_%s_JHUGen.root",
      strChannel.Data(), strCategory.Data(),
  ...
  gSystem->Exec("mkdir -p " + coutput_common);
  TString OUTPUT_NAME = Form(
    "HtoZZ%s_%s_FinalTemplates_%s_%s_JHUGen_Check%sDiscriminants",
    strChannel.Data(), strCategory.Data(),
