...
ExtendedBinning getIntermediateBinning(TH1F const* fineHisto, ExtendedBinning const& finalBinning, int effectiveN){
  ExtendedBinning res;
  TString bnamelow = finalBinning.getLabel(); bnamelow.ToLower();
  //if (bnamelow.Contains("mass")){
    ...
    else effNThreshold=effectiveNtotal/double(finalBinning.getNbins());
    ...
    */
    ...
    unsigned int nbinsreq=finalBinning.getNbins();
    nbinsreq *= 2./3.;
    res = ExtendedBinning(nbinsreq, finalBinning.getMin(), finalBinning.getMax(), finalBinning.getLabel());
  }
  */
  return res;
}

void getIntermediateBinning(TH2F const* fineHisto, ExtendedBinning const& finalXBinning, ExtendedBinning const& finalYBinning, ExtendedBinning& xbinning, ExtendedBinning& ybinning, int effectiveNx, int effectiveNy){
  ...
  xbinning=getIntermediateBinning(fineHistoX, finalXBinning, effectiveNx);
  ybinning=getIntermediateBinning(fineHistoY, finalYBinning, effectiveNy);

  delete fineHistoY;
  delete fineHistoX;
}

void getIntermediateBinning(TH3F const* fineHisto, ExtendedBinning const& finalXBinning, ExtendedBinning const& finalYBinning, ExtendedBinning const& finalZBinning, ExtendedBinning& xbinning, ExtendedBinning& ybinning, ExtendedBinning& zbinning, int effectiveNx, int effectiveNy, int effectiveNz){
  // Do not include overflow/underflow bins; they should not be used for determining the intermediate binning, which doesn't cover the under/overflows.
  ...
  xbinning=getIntermediateBinning(fineHistoX, finalXBinning, effectiveNx);
  ybinning=getIntermediateBinning(fineHistoY, finalYBinning, effectiveNy);
  zbinning=getIntermediateBinning(fineHistoZ, finalZBinning, effectiveNz);
...
TH1F* getSmoothHistogram(
  ...
  ExtendedBinning intermediateXBinning = getIntermediateBinning(res, finalXBinning, effectiveN);
  delete res;
...
TH2F* getSmoothHistogram(
  ...
  getIntermediateBinning(res.getHistogram(), finalXBinning, finalYBinning, intermediateXBinning, intermediateYBinning, effectiveNx, effectiveNy);
  ...
  if (!smoothCondX && !smoothCondY){
    ...
    for (unsigned int ix=0; ix<=intermediateXBinning.getNbins()+1; ix++){
      MELAout << "\t- Constructing slice " << ix;
      if (ix==0) MELAout << " (x<" << intermediateXBinning.getBinLowEdge(0) << ")";
      else if (ix==intermediateXBinning.getNbins()+1) MELAout << " (x>" << intermediateXBinning.getBinLowEdge(intermediateXBinning.getNbins()) << ")";
      else MELAout << " (x: [ " << intermediateXBinning.getBinLowEdge(ix-1) << " , " << intermediateXBinning.getBinLowEdge(ix) << " ]";
      MELAout << endl;

      TH1F* hProj = getHistogramSlice(res.getHistogram(), 1, ix, ix, "hProj");
      {
        if (ix==1){
          cout << "UNREBINNED: " << endl;
          for (int i=0; i<=hProj->GetNbinsX()+1; i++){
            cout
              << "Bin " << i
              << " val/error = " << hProj->GetBinContent(i) << " / " << hProj->GetBinError(i)
              << endl;
          }
        }
      }
      ExtendedBinning intermediateBinning = getIntermediateBinning(hProj, finalYBinning, effectiveNy);
      MELAout << "\t- Intermediate Y binning: [ " << intermediateBinning.getBinningVector() << " ]" << endl;
      MELAout << "\t- Initial rebinning of hProj to intermediate binning" << endl;
      ...
      MELAout << "\t- Initial rebinning of hProj to final binning" << endl;
      rebinHistogram(hProj, finalYBinning);
      {
        if (ix==1){
          cout << "REBINNEDx2: " << endl;
          for (int i=0; i<=hProj->GetNbinsX()+1; i++){
            cout
              << "Bin " << i
              << " val/error = " << hProj->GetBinContent(i) << " / " << hProj->GetBinError(i)
    ...
    for (unsigned int iy=0; iy<=intermediateYBinning.getNbins()+1; iy++){
      ...
      ExtendedBinning intermediateBinning = getIntermediateBinning(hProj, finalXBinning, effectiveNx);
