...
void produce_MJJdistributions_one(int folder, int erg_tev){
  ...
  char TREE_NAME[] = "SelectedTree";
	TString OUTPUT_NAME = "HtoZZ4l_Phantom_125p6_VHDistributions";
	TString comstring;
 ...
 //   TString coutput_common = "./data/VHContributions/" + erg_dir;
...
//    const int nbinsy = 260;
 ...
 //   float kDY_bounds[2] ={ 0, 130.5 };
      ...
      */
      ...
      for (int decay = 0; decay < 3; decay++){
        ...
        if (decay == 2) channelname = "2e2mu";
				...
				TString cinput_VBF = cinput_common + "HZZ4lTree_ZZTo" + channelname + "JJ_" + sample_suffix_Phantom[tr] + "_Reprocessed.root";
    ...
    double ratioW = WH_Sig_Datacard[EnergyIndex][folder] / (simulatedW-simulatedWbkg);
    double ratioZ = ZH_Sig_Datacard[EnergyIndex][folder] / (simulatedZ-simulatedZbkg);
    ...
    double vbfscale = VBF_Sig_Datacard[EnergyIndex][folder] / nVBF_Sig_Simulated;
...
void plotVBFVHReweighted(){
  ...
  TString INPUT_NAME = "HtoZZ4l_Phantom_125p6_VHDistributions.root";
  TString OUTPUT_NAME = "HtoZZ4l_Phantom_125p6_VHReweightingPlots.root";
  ...
  TString coutput_common = user_TemplateswithTrees_dir + "../VHContributions/Plots/";
  gSystem->Exec("mkdir -p " + coutput_common);
  TString coutput = coutput_common + OUTPUT_NAME;
  TFile* foutput = new TFile(coutput, "recreate");
...
//  TString strmzztitle[2]={ "105.6<m_{4l}<140.6 GeV", "220<m_{4l}<1600 GeV" };
  ...
  for (int os=0; os<2; os++){
    for (int bsi=0; bsi<3; bsi++){
      ...
      TString canvasname = "cCompareMJJ";
